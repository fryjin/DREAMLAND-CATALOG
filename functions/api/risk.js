...
function evaluateRisk(request,body){
  ...
  if(asNumber(risk.session_elapsed_ms)<15000){
    add(2,'very_fast_session');
  }

  if(asNumber(risk.form_elapsed_ms)<8000){
    add(2,'very_fast_form');
  }

  if(asNumber(risk.interaction_count)<3){
    add(1,'few_interactions');
  }

  if(asNumber(risk.local_attempt_count)>=2){
