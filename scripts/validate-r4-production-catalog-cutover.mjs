...
if(DIST_MODE){
  try{
    ...
    if(
      ...
      fail(
        'R4.4C downstream Contact route is missing after the R4.8C cutover.'
      );
    }else{
      const contact=
        fs.readFileSync(
      ...
      if(
        ...
        fail(
          'R4.4C downstream owner compatibility requires Astro Contact after R4.8C.'
        );
      }
    }
    ...
    for(const relative of [
      ...
      if(
        ...
        fail(
          'R4.4C must preserve Legacy Review/Success ownership: '+
          relative
        );
      }
    }
